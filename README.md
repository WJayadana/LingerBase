...
> Status: production-ready architecture scaffold. OCR and translation now fail loudly when their external engines are not installed/configured, so the CLI will not silently export unchanged pages as “translated”.
...
```bash
OPENAI_API_KEY=... uv run manga translate input.cbz
...
Install an Unlimited-OCR executable available as `unlimited-ocr`, or set `[ocr].command` to its path. For OpenAI translation, export `OPENAI_API_KEY` before running the CLI. Use `provider = "noop"` only for explicit dry-runs/tests because it intentionally preserves source text.
